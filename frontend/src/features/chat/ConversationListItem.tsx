...
function ConversationListItemInner({ conversation }: { conversation: ConversationSummary }) {
  ...
  return (
    <div ref={wrapRef} className="group relative">
      ...
      {menuOpen && (
        <div className="absolute right-2 top-10 z-20 overflow-hidden rounded-lg border border-slate-200 bg-white text-sm shadow-xl dark:border-white/10 dark:bg-[#202c33]">
          <button
            onClick={handleClear}
